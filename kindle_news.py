...
# URLs de los feeds RSS (corregidas)
DIARIOS_MEXICO = {
...
def generar_periodico_html():
    ...
    for nombre, url in DIARIOS_MEXICO.items():
        ...
        try:
            ...
            if not feed.entries:
                print(f"⚠️ Sin noticias disponibles para {nombre}")
...
def enviar_a_kindle(contenido_html):
    """Envía el archivo HTML al Kindle usando Brevo SMTP"""
    ...
    # Conectar con Brevo SMTP
    print("🔌 Conectando con servidor SMTP de Brevo...")
    server = smtplib.SMTP("smtp-relay.brevo.com", 587)
    server.starttls()

    try:
        # Autenticación con Brevo
        print(f"📧 Autenticando con {EMAIL_EMISOR}...")
